    ...
    try:
        ...
        for user in users:
            try:
                ...
                user_id = user['id']
                raw_frequency = customer.get('frequency', 'week')
                interval = get_interval_from_frequency(raw_frequency)
                ...
                if last_sent_data is None:
                    ...
                    logger.info(f"Created tracker entry for {email} - sending first email")
                    should_send = True
                else:
                    last_status = last_sent_data.get("last_status")
                    last_sent_time = datetime.fromisoformat(last_sent_data["last_sent"]).replace(tzinfo=utc)
                    
                    stuck_threshold = timedelta(minutes=10)
                    if last_status == "in_progress":
                        logger.info(f"Skipping {email} - email already in progress")
                        continue
                    elif last_status == "queued":
                        if (now_utc - last_sent_time) < stuck_threshold:
                            logger.info(f"Skipping {email} - recently queued")
                            continue
                        else:
                            logger.warning(f"Retrying stuck queued job for {email}")
                            should_send = True
                    else:
                        send_hour_start = customer.get('send_hour_start', 11)
                        send_hour_end = customer.get('send_hour_end', 12)
                        if not (send_hour_start <= current_hour_ist < send_hour_end):
                            logger.info(f"Skipping {email} - not in send window")
                            continue
                        last_sent_date = last_sent_time.astimezone(IST).date()
                        if last_sent_date == current_date_ist:
                            logger.info(f"Already sent to {email} today")
                        elif interval > 0 and (current_date_ist - last_sent_date).days < interval:
                            logger.info(f"Interval not passed for {email}")
                        elif (now_ist - last_sent_time.astimezone(IST)) < timedelta(minutes=5):
                            logger.info(f"Safety skip for {email}")
                        else:
                            should_send = True
                ...
                if should_send:
                    ...
                    msg = run_user_job.send(user_id, email, customer.get('company_names', ''), interval=raw_frequency)
                    logger.info(f"Enqueued Dramatiq job → {email}, msg_id={msg.message_id}")
                    emails_queued += 1
